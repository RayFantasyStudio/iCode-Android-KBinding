...
object PostUtil {
     ...
     * *
     */
    fun insertCodeGood(codeGood: CodeGood, init: OnSuccessListener.() -> Unit): Request<*>? {
        val listener = OnSuccessListener().apply(init)
    ...
    fun delCodeGood(id: Int, init: OnSuccessListener.() -> Unit): Request<*>? {
        val listener = OnSuccessListener().apply(init)
        if (user == null || key == null) {
            listener.onFailed(PostException("登陆后才能进行此操作"), -4)
            return null
     ...
     * *
     */
    fun selectCodeGood(condition: String, init: OnSuccessListener1<MutableList<CodeGood>>.() -> Unit): Request<*> {
     ...
     * *
     */
    fun loadCodeContent(id: Int, init: OnSuccessListener1<String?>.() -> Unit): Request<*> {
     ...
     * *
     */
    fun addReply(reply: Reply, init: OnSuccessListener.() -> Unit): Request<*>? {
        val listener = OnSuccessListener().apply(init)
        if (user == null || key == null) {
            listener.onFailed(PostException("登陆后才能进行此操作"), -4)
            return null
    ...
    fun delReply(id: Int, init: OnSuccessListener.() -> Unit): Request<*>? {
        val listener = OnSuccessListener().apply(init)
        if (user == null || key == null) {
            listener.onFailed(PostException("登陆后才能进行此操作"), -4)
            return null
     ...
     * *
     */
    fun findReply(condition: String, init: OnSuccessListener1<MutableList<Reply>>.() -> Unit): Request<*> {
    ...
    fun addFavorite(goodId: Int, init: OnSuccessListener.() -> Unit): Request<*>? {
        val listener = OnSuccessListener().apply(init)
        if (user == null || key == null) {
            listener.onFailed(PostException("登陆后才能进行此操作"), -4)
            return null
    ...
    fun findFavorite(init: OnSuccessListener1<MutableList<Favorite>>.() -> Unit): Request<*>? {
        val listener = OnSuccessListener1<MutableList<Favorite>>().apply(init)
        if (user == null || key == null) {
            listener.onFailed(PostException("登陆后才能进行此操作"), -4)
            return null
        }
    ...
    fun delFavorite(goodId: Int, init: OnSuccessListener.() -> Unit): Request<*>? {
        val listener = OnSuccessListener().apply(init)
        if (user == null || key == null) {
            listener.onFailed(PostException("登陆后才能进行此操作"), -4)
            return null
    ...
    fun registerUser(username: String, password: String, init: OnSuccessListener1<User>.() -> Unit): Request<*> {
        val listener = OnSuccessListener1<User>().apply(init)
        val data = JSONObject()
    ...
    fun loginUser(username: String, password: String, init: OnSuccessListener1<User>.() -> Unit): Request<*> {
        val listener = OnSuccessListener1<User>().apply(init)
        val data = JSONObject()
        data.put("username", username)
        data.put("password", password)
    ...
    fun resetPwd(oldPwd: String, newPwd: String, init: OnSuccessListener1<User>.() -> Unit): Request<*>? {
        val listener = OnSuccessListener1<User>().apply(init)
        if (user == null) {
            listener.onFailed(PostException("登陆后才能进行此操作"), -4)
            return null
    ...
    fun uploadProfilePic(pic: File, init: OnSuccessAndProgressListener.() -> Unit): RequestHandle? {
        val listener = OnSuccessAndProgressListener().apply(init)
        if (user == null || key == null) {
            listener.onFailed(PostException("登陆后才能进行此操作"), -4)
            return null
        }
